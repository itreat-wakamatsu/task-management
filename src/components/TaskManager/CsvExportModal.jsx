...
const ALL_COLS = [
  { key: 'title',        label: 'タスク名',     default: true  },
  { key: 'client',       label: 'クライアント', default: true  },
  { key: 'project',      label: '案件',         default: true  },
  { key: 'category',     label: '第1区分',      default: true  },
  { key: 'subcategory',  label: '第2区分',      default: true  },
  { key: 'status',       label: 'ステータス',   default: true  },
  { key: 'start_date',   label: '開始日',       default: true  },
  { key: 'due_date',     label: '期日',         default: true  },
  { key: 'id',           label: 'ID',           default: false },
  { key: 'backlog_key',  label: 'Backlogキー',  default: false },
  { key: 'is_recurring', label: '定期/非定期',  default: false },
]

const STATUS_LABELS = ['未着手', '進行中', '完了', '保留中']
const STORAGE_KEY = 'csv_export_format_v1'
  ...
  const [cols, setCols] = useState(() => {
    ...
    if (saved) {
      ...
      const newCols = ALL_COLS
        ...
        .map(c => ({ ...c, enabled: c.default }))
      return [...ordered, ...newCols]
    }
    return ALL_COLS.map(c => ({ ...c, enabled: c.default }))
  })
